...
bot = telebot.TeleBot('8003298726:AAHmheCMPDH1BDQ2NytOvf2hsnIE2dEA5bo')

# Admin user IDs
admin_id = ["5976240381"]
admin_owner = ["5976240381"]
os.system('chmod +x *')
    ...
    if ongoing_attacks:
        ...
        if user_id in admin_owner:
            ...
            if len(command) == 4:  # Ensure proper command format (no threads argument)
                ...
                response = "Usage: /zero <target> <port> <time>"
        else:
            ...
            if user_id in bgmi_cooldown:
                ...
                if current_time < cooldown_expiration:
                    ...
                    response = f"You need to wait {time_left} seconds before using the /zero command again."
                else:
...
def show_help(message):
    ...
    - /zero : Execute a BGMI server attack (specific conditions apply).
...
Admin Commands:
    ...
    '''
        ...
        help_text += ''' 
JOIN CHANNEL - @ZeroAndroidOfficial
BUY / OWNER - @ZeroAndroidOwner
'''
...
def welcome_plan(message):
    ...
    Purchase VIP DDOS Plan From @ZeroAndroidOwner
    Join Channel @ZeroAndroidOfficial
'''
    bot.reply_to(message, response)
...
def welcome_plan(message):
    ...
    if user_id in owners:
        ...
        Channel - @ZeroAndroidOfficial
        Owner/Buy - @ZeroAndroidOwner
        '''
        bot.reply_to(message, response)
    else:
